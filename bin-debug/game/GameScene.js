...
var GameScene = (function (_super) {
    ...
    GameScene.prototype.initView = function () {
        // 背景
        var bg = GameUtil.createBitmapByName('bg_game_png');
    ...
    // 初始化砖块层
    GameScene.prototype.initBrickContainer = function () {
        // 读取文件中的砖块数据
        ...
        for (var i = 0; i < brickCount; i++) {
            var brickItem = new Brick(this.stage.stageWidth / 5, 60);
    ...
    // 	 * 后面再次点击 跳跃
    // 	 */
    GameScene.prototype.onClickView = function (evt) {
        // 点击屏幕，游戏开始，ball跳起来
    ...
    GameScene.prototype.collision = function () {
        ...
        //碰撞检测
        var ball_rect = new egret.Rectangle(ball.x, ball.y, ball.width, ball.height);
        var _loop_1 = function (item) {
            // 游戏对象列表中包含砖块和木板，拿出来观察是否有撞击到砖块和木板
            // 如果撞击到的是砖块
            if (item instanceof Brick) {
                var item_rect = new egret.Rectangle(item.x, item.y, item.width, item.height);
                if (item_rect.intersects(ball_rect) && !item.hasTrigger) {
                    ...
                    this_1.deleteObjectList.push(item);
                }
            }
            // 如果撞击到的是木板
            if (item instanceof Board) {
                var board_rect = new egret.Rectangle(item.x, item.y, item.width, item.height);
            ...
            // 删除本回合中被消去的砖块
            for (var _i = 0, _a = this_1.deleteObjectList; _i < _a.length; _i++) {
                var obj = _a[_i];
                this_1.brickContainer.removeChild(obj);
                this_1.gameObjectList.splice(this_1.gameObjectList.indexOf(obj), 1);
            }
            this_1.deleteObjectList.length = 0;
        };
        var this_1 = this;
